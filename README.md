...
- 共享句对记忆库：选中片段后按源文相似度列出可复用的旧译文（相似度百分比、高度/部分/弱相似分级与命中关键词），低于门槛的条目不展示；采用后旧译文填入当前片段并退回草稿，修改历史写明取自哪条记忆。源文改动后采用记录会标出对不上当前源文并按新源文重新建议；不需要的条目可停用且随草稿保存。
- 修改历史包含编辑、确认、退回、冲突处理、记忆采用和导入记录；可解决本地与远端翻译冲突。
