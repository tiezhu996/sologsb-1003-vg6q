...
import { seedConflicts, seedDocument, seedHistory, seedMemory } from '@/lib/seed'
...
export const handlers = [
  ...
  http.get('/api/memory', () => HttpResponse.json(clone(seedMemory))),
  ...
  http.post('/api/draft', async ({ request }) => {
    const body = await request.json() as { documentId: string; segments: Segment[]; discussions: unknown[]; memoryBank?: unknown[] }
    await new Promise((resolve) => setTimeout(resolve, 240))
    return HttpResponse.json({ saved: true, documentId: body.documentId, segmentCount: body.segments.length, memoryCount: body.memoryBank?.length ?? 0, savedAt: Date.now() })
  }),
